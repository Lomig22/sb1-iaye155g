...
import { Plus, Mail, AlertCircle, Clock, Edit, Search, Trash2, Upload, X, Check as CheckIcon, Info } from 'lucide-react';
...
function ReceivablesList() {
  ...
  const [receivableToDelete, setReceivableToDelete] = useState<(Receivable & { client: Client }) | null>(null);
  ...
  const checkClientUnpaidReceivables = async (clientId: string): Promise<boolean> => {
    try {
      const { data, error } = await supabase
        .from('receivables')
        .select('id')
  ...
  const handleSendReminder = async (receivable: Receivable & { client: Client }) => {
    try {
      ...
      if (success) {
        ...
        setError('Impossible d\'envoyer la relance. Vérifiez les paramètres email et les templates.');
      }
    } catch (error: any) {
                          ...
                          {(!receivable.client.reminder_template_1 && receivable.client.needs_reminder) && (
                            <div className="relative">
                              <div 
                                ...
                                onMouseEnter={() => handleMouseEnter(receivable.id)}
                              ...
                              {tooltipVisible === receivable.id && (
                                <div className="absolute z-10 w-64 px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm -left-32 bottom-full mb-1">
                                  Paramètres de relance non configurés. Cliquez sur l'icône d'horloge pour configurer les modèles de relance.
                                </div>
                              )}
                            </div>
                          )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Êtes-vous sûr de vouloir supprimer la créance "{receivableToDelete.invoice_number}" pour le client "{receivableToDelete.client.company_name}" ?
