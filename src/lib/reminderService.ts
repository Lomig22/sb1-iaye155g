...
function determineReminderLevel(
  daysLate: number,
  client: Client
): { level: 'first' | 'second' | 'third' | 'final' | null; template: string | null } {
  if (!client) return { level: null, template: null };

  if (daysLate >= (client.reminder_delay_final || 60) && client.reminder_template_final) {
    return { level: 'final', template: client.reminder_template_final };
  }
  if (daysLate >= (client.reminder_delay_3 || 45) && client.reminder_template_3) {
    return { level: 'third', template: client.reminder_template_3 };
  }
  if (daysLate >= (client.reminder_delay_2 || 30) && client.reminder_template_2) {
    return { level: 'second', template: client.reminder_template_2 };
  }
  if (daysLate >= (client.reminder_delay_1 || 15) && client.reminder_template_1) {
...
export async function sendManualReminder(receivableId: string): Promise<boolean> {
  try {
